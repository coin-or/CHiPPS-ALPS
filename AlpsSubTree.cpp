...
// which means the master already solve the problem). During the course, 
// master may find solutions, and later it will let workers know the solutions.
void 
AlpsSubTreeMaster::initializeSearch(AlpsTreeNode* root) 
{
   AlpsTreeNode* node;
   ...
   //  while (! nodepool_->empty()) {
   while(! nodepool_->empty() && nodepool_->getNumKnowledges() < 
	 AlpsData::parameters()->entry(AlpsAllParam::intialNodeNum) ) { 
