...
AlpSubTree:: doOneUnitWork()
{
  ...
  const double unitTime =
    AlpsDataPool::getOwnParams()->entry(AlpsOwnParams::unitWorkTime);
...
void
AlpSubTree::unpackSubTree(char* buf) 
{
...
AlpsSubTreeHub::rampUp() 
{
   ...
   const int nodeNum =  AlpsDataPool::getOwnParams()->
     entry(AlpsOwnParams::hubInitNodeNum);
...
AlpsSubTreeMaster::rampUp(AlpsTreeNode* root) 
{
   ...
   nodepool_->addKnowledge(root_);
   const int nodeNum =  
     AlpsDataPool::getOwnParams()->entry(AlpsOwnParams::masterInitNodeNum);
