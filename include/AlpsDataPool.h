...
class AlpsDataPool : public AlpsKnowledgePool {
 ...
 public:
   ...
   void addKnowledge(const AlpsKnowledgeType kt, AlpsKnowledge* kg) { 
    ...
    /** Add static data into data pool. */
   void addKnowledge(const AlpsKnowledge * nk, double priority=0) { 
     throw CoinError("Can not call addKnowledge()",
		     "addKnowledge()", "AlpsDataPool");
   }

   /** Update the value of kt if kt is not exit; otherwise call addKnowledge().
    */
   void updateKnowledgeValue(const AlpsKnowledgeType kt, AlpsKnowledge* kg){
