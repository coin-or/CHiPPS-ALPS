...
// AlpsEncoded is modified from BCP_buffer.hpp and CoinEncoded.h.

//#############################################################################

/** Type of knowledge like solution, node, cut...*/
enum AlpsKnowledgeType { MODEL, NODE, SOLUTION };
...
private:
 ...
 public:
  ...
  AlpsEncoded(char*& t, const int s, char*& r) 
    ...
    {
      t = NULL;  r = NULL; // "new" should pair with "delete".
    }                      // Need pack, unpack?
  ~AlpsEncoded() {
  ...
  //  inline void setPosition(const int pos) throw(AlpsException) {
  ...
  template <class T> AlpsEncoded& unpack(T& value){
    ...
    if (pos_ + sizeof(T) > size_) {
      //     const char msg [100] = "Reading over the end of buffer.";
      //throw AlpsException(__FILE__, __LINE__, msg);
   ...
   */
   template <class T> AlpsEncoded& unpack(T*& values, int& length,
	 ...
	 throw CoinError("Reading over the end of buffer.", 
       ...
       pos_ += sizeof(int);
       if (l != length) {
	 //	 const char msg [100] = "unpack() : bad array lentgh.\n";
	 //throw AlpsException(__FILE__, __LINE__, msg);
	 throw CoinError("Reading over the end of buffer.", 
			 "AlpsEncoded", "unpack(T*& values, int& le..,...");
       }
...
class AlpsKnowledge {
 ...
 public:
  ...
  AlpsKnowledge() : encoded_(0) {
    ...
    //   decodeMap_ = new std::map<const char*, const AlpsKnowledge*,
    //				 AlpsStrLess>;
      //   }
  }
  ...
  virtual AlpsEncoded* encode() const = 0;
  ...
  virtual AlpsKnowledge* decode(AlpsEncoded&) const = 0;
...
// FIXME: Multiple definition error in Knap Example. Put in .cpp file!
