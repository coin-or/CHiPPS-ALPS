...
//  CoinSharedPtr<AlpsNodePool*> nodepool_;
 ...
 public:
   ...
   AlpsNodePool* getNodePool() { return nodepool_; }
   void setNodePool( AlpsNodePool* np) { nodepool_ = np; }

   AlpsSolutionPool* getSolutionPool() { return solpool_; }
   void setNodePool( AlpsSolutionPool* np) { solpool_ = np; }
};
...
class AlpsSubTreeMaster : public AlpsSubTreeWorker {

 public:
  /** Master create enough nodes for workers and distribution them among
      workers. */
