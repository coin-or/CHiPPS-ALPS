...
AlpsKnowledgeBrokerMPI::masterMain(AlpsTreeNode* root)
{
  ...
  const double period    = 
    AlpsData::parameters()->entry(AlpsOwnParam::masterBalancePeriod);
  const bool interCB     = 
    AlpsData::parameters()->entry(AlpsOwnParam::interClusterBalance);
  ...
  const int largeSize    = 
    AlpsData::parameters()->entry(AlpsOwnParam::largeSize);
  const int medSize      = 
    AlpsData::parameters()->entry(AlpsOwnParam::mediumSize);
  const int smallSize    = 
    AlpsData::parameters()->entry(AlpsOwnParam::smallSize);

  const double zeroLoad  = 
    AlpsData::parameters()->entry(AlpsOwnParam::zeroLoad);
...
AlpsKnowledgeBrokerMPI::hubMain()
{
  ...
  const int smallSize    = 
    AlpsData::parameters()->entry(AlpsOwnParam::smallSize);
  const double period    = 
    AlpsData::parameters()->entry(AlpsOwnParam::hubReportPeriod);
  const double zeroLoad  = 
    AlpsData::parameters()->entry(AlpsOwnParam::zeroLoad); 
...
AlpsKnowledgeBrokerMPI::workerMain()
{
  ...
  const double workerAP  = 
    AlpsData::parameters()->entry(AlpsOwnParam::workerAskPeriod); 
  const double zeroLoad  = 
    AlpsData::parameters()->entry(AlpsOwnParam::zeroLoad); 

  const bool intraCB     = 
    AlpsData::parameters()->entry(AlpsOwnParam::intraClusterBalance);
...
AlpsKnowledgeBrokerMPI::askHubsShareWork(int donorID, 
					 ...
					 MPI_Comm comm)
{
  int smallSize = AlpsData::parameters()->entry(AlpsOwnParam::smallSize);
  int pos       = 0;
...
{
  const int size = AlpsData::parameters()->entry(AlpsOwnParam::keyNodeNum);
...
AlpsKnowledgeBrokerMPI::donateWork(char* buf, 
				   ...
				   MPI_Comm comm)
{
  int size       = AlpsData::parameters()->entry(AlpsOwnParam::smallSize);
  int pos        = 0;
  int receiverID = 0;
  char* dummyBuf = 0;
  ...
  const int minNodeNum = 
    AlpsData::parameters()->entry(AlpsOwnParam::minNodeNum);
					  ...
					  MPI_Comm comm)
{
  ...
  double  maxLoad = AlpsData::parameters()->entry(AlpsOwnParam::zeroLoad);
...
AlpsKnowledgeBrokerMPI::masterBalanceHubs(MPI_Comm comm)
{
  ...
  const double donorSh      = AlpsData::parameters()->
    entry(AlpsOwnParam::donorThreshold);
  const double receiverSh   = AlpsData::parameters()->
    entry(AlpsOwnParam::receiverThreshold);
  const double zeroLoad     = 
    AlpsData::parameters()->entry(AlpsOwnParam::zeroLoad);
				       ...
				       MPI_Comm comm)
{
  ...
  if (id == source) {
    ...
    packEncoded((AlpsData::getModel())->encode(), buf, size, position);
  }
...
{
  int i         = -1;
  //  const int pn  = AlpsData::parameters()->entry(AlpsOwnParam::processNum);
  int size      = AlpsData::parameters()->entry(AlpsOwnParam::smallSize);
  int position  = 0;
  char* buf     = new char [size];
					    ...
					    MPI_Comm comm)
{
  ...
  if ( sender == destination ) {
	...
	delete [] senderBuf;     senderBuf = 0;
      ...
      double value = 0.0;
      // int ls = AlpsData::parameters()->entry(AlpsOwnParam::largeSize);
...
AlpsKnowledgeBrokerMPI::packEncoded(AlpsEncoded* enc, 
				    ...
				    int& position) 
{
  const int bufSpare = AlpsData::parameters()->entry(AlpsOwnParam::bufSpare);
...
AlpsKnowledgeBrokerMPI::unpackEncoded(char*& buf) 
{
  ...
  int typeSize;
  int repSize; 
  int size = AlpsData::parameters()->entry(AlpsOwnParam::largeSize);
...
AlpsKnowledgeBrokerMPI::receiveBuf(char*& buf, 
				   ...
				   MPI_Status* status) 
{
  const int size = AlpsData::parameters()->entry(AlpsOwnParam::largeSize);
  if (buf != 0) {
    delete [] buf;  buf = 0;
  }
  buf = new char [size];
					 ...
					 AlpsParameterSet& userParams) 
{
  ...
  if (globalRank_ == masterRank_) {
    ...
    if (AlpsData::parameters()->entry(AlpsOwnParam::inputFromFile) == true) {
      const char* dataFile = 
	AlpsData::parameters()->entry(AlpsOwnParam::dataFile).c_str();
    ...
    std::cout << "MASTER: num of process = " 
	      << AlpsData::parameters()->entry(AlpsOwnParam::processNum)
	      << std::endl;
#endif
  ...
  hubNum_ = AlpsData::parameters()->entry(AlpsOwnParam::hubNum);
...
AlpsKnowledgeBrokerMPI::solve(AlpsTreeNode* root)
{
  ...
  if (processType_ == AlpsProcessTypeMaster) {
    ...
    const int mns = 
      AlpsData::parameters()->entry(AlpsOwnParam::maxNumSolustion);
