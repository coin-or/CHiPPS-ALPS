...
static Alps_message us_english[] =
{
    ...
    {ALPS_LOADBAL_HUB, 5, 3, "Hub[%d] balanced the workload of its workers %d times"},
    {ALPS_LOADBAL_HUB_FAIL, 6, 1, "Hub[%d] failed to find a process to donate work to process %d"},
    {ALPS_LOADBAL_HUB_PERIOD, 7, 1, "Hub[%d] initially balances or report its workers every %.4f seconds"},
    {ALPS_LOADBAL_MASTER, 8, 3, "Master[%d] balanced the workloads of the hubs %d times"},
    {ALPS_LOADBAL_MASTER_NO, 9, 1, "Master[%d] balanced workload but do nothing since work load is %g"},
    {ALPS_LOADBAL_MASTER_PERIOD, 11, 1, "Master[%d] initially balances the hubs every %.4f seconds"},
    {ALPS_LOADBAL_WORKER_ASK, 12, 3, "Worker[%d] asks its hub (%d) for work"},
    {ALPS_LOADREPORT_MASTER, 111, 1, "Node %d: left %g, msg(sent %d, received %d), inter %d, incumbent %g, wallclock %.0f sec."},
    {ALPS_LOADREPORT_MASTER_F, 112, 1, "Node %d: left(%g / %g), msg(sent %d, received %d), inter %d, incumbent %g, wallclock %.0f sec."},
    {ALPS_LOADREPORT_MASTER_N, 115, 1, "Node %d: left %g, msg(sent %d, received %d), inter %d, no solution, wallclock %.0f sec."},
    {ALPS_LOADREPORT_MASTER_F_N, 116, 1, "Node %d: left(%g / %g), msg(sent %d, received %d), inter %d, no solution, wallclock %.0f sec."},
    {ALPS_MSG_HOW, 14, 3, "Process[%d] %s count %d in %s"},
    {ALPS_NODE_COUNT, 16, 3, "Worker[%d] processed %d nodes, %d nodes are left"},
    {ALPS_NODE_MEM_SIZE, 17, 1, "Approximately memory size of a node is %d bytes"},
    {ALPS_P_VERSION, 200, 1, "ALPS version 0.9.5 (Parallel, MPI)"},
    {ALPS_PARAMFILE, 18, 1, "Parameter file: %s"},
    {ALPS_RAMPUP_HUB, 19, 1, "Hub[%d]'s rampup took %g seconds to process %d nodes. Node pool has %d nodes"},
    ...
    {ALPS_RAMPUP_HUB_START, 23, 1, "Hub[%d] is creating nodes (%d) for its workers during rampup"},
    {ALPS_RAMPUP_MASTER, 24, 1, "Master[%d]'s rampup took %g seconds to process %d nodes. Node pool has %d nodes"},
    ...
    {ALPS_RAMPUP_MASTER_START, 27, 1, "Master[%d] is creating nodes (%d) for its hubs during rampup"},
